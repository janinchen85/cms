<div class="row card-body bg-dark border_bottom">
    <div class="col-md-5">
            <a href="../thread/{$threadID}">{$threadTitle}</a>
        <br>
        <span style="font-size:12px">by:
            <a href="../user/{$userID}">{$userName}</a> on {$threadDate} - {$threadTime}</span>
    </div>
    ...
    <div class="col-md-4" style="font-size:14px">by: {$postUserName}
        <br>
        <span style="font-size:12px">on {$postDay}, {$postDate} - {$postTime}</span>
    </div>
</div>